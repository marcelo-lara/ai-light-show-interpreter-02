...
- **STRICT READ-ONLY SYSTEM:** For the `ai-light-show-interpreter-02`, ALL files and directories within the `/data` folder are strictly **READ-ONLY**, except for the `data/shows/` directory where compiled show files are output. This system must never mutate source audio, artifacts, or intelligence outputs.
